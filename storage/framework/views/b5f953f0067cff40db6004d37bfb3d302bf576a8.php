        ...
        <div class="col-12 col-md-3">
            <div class="position-relative bg-info-light d-flex flex-column align-items-center justify-content-start rounded-lg w-100 h-100 p-20">
                <div class="user-avatar rounded-circle <?php echo e(($cardUser->id == $topic->creator_id) ? 'green-ring' : ''); ?>">
                    <img src="<?php echo e(asset($cardUser->getAvatar(72))); ?>" class="img-cover rounded-circle" alt="<?php echo e($cardUser->full_name); ?>">
                </div>
                <a href="<?php echo e(url($cardUser->getProfileUrl())); ?>" target="_blank">
                    ...
                    <div class="d-flex flex-wrap align-items-center justify-content-center mt-20 w-100">
                        <?php $__currentLoopData = $cardUserBadges; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $badge): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="mr-10 mt-10" data-toggle="tooltip" data-placement="bottom" data-html="true" title="<?php echo (!empty($badge->badge_id) ? nl2br($badge->badge->description) : nl2br($badge->description)); ?>">
                                <img src="<?php echo e(url(!empty($badge->badge_id) ? $badge->badge->image : $badge->image)); ?>" width="32" height="32" alt="<?php echo e(!empty($badge->badge_id) ? $badge->badge->title : $badge->title); ?>">
                            </div>
                ...
                <?php if(!empty($post) and $post->pin): ?>
                    <span class="pinned-icon d-flex align-items-center justify-content-center">
                        <img src="<?php echo e(asset('')); ?>assets/default/img/learning/un_pin.svg" alt="pin icon" class="">
                    </span>
                <?php endif; ?>
            </div>
        </div>
                ...
                <div class="d-flex flex-column h-100">
                        ...
                        <div class="post-quotation p-15 rounded-sm border mb-15">
                            <div class="d-flex align-items-center">
                                <div class="post-quotation-icon rounded-circle">
                                    <img src="<?php echo e(asset('')); ?>assets/default/img/icons/quote-right.svg" class="img-cover" alt="quote-right">
                                </div>
                                <div class="ml-10">
                                    <span class="d-block font-12 text-gray">Balas ke</span>
                    ...
                    <?php if(!empty($post) and !empty($post->attach)): ?>
                        <div class="mt-auto d-inline-flex">
                            <a href="<?php echo e(url($post->getAttachmentUrl($forum->slug,$topic->slug))); ?>" target="_blank" class="d-flex align-items-center text-gray bg-info-light border px-10 py-5 rounded-pill">
                    ...
                    <?php elseif(empty($post) and !empty($topic->attachments) and count($topic->attachments)): ?>
                        <div class="mt-auto d-inline-flex align-items-center">
                            <?php $__currentLoopData = $topic->attachments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $attachment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <a href="<?php echo e(url($attachment->getDownloadUrl($forum->slug,$topic->slug))); ?>" target="_blank" class="d-flex align-items-center text-gray bg-info-light border px-10 py-5 rounded-pill mr-15">
                                    <i data-feather="paperclip" class="text-gray" width="16" height="16"></i>
                    ...
                    <div class="d-flex align-items-center">
                        <?php if(!empty($authUser) and !$topic->close): ?>
                            <?php if($authUser->id == $cardUser->id): ?>
                                <?php if(!empty($post)): ?>
                                    <button type="button" data-action="<?php echo e(url($post->getEditUrl($forum->slug,$topic->slug))); ?>" class="js-post-edit btn-transparent mr-25 font-14 font-weight-500 text-gray">Edit</button>
                                <?php else: ?>
                                    <a href="<?php echo e(url($topic->getEditUrl($forum->slug))); ?>" class="mr-25 font-14 font-weight-500 text-gray">Edit</a>
                                <?php endif; ?>
                            <?php endif; ?>

                            <?php if(!empty($post) and $authUser->id == $topic->creator_id): ?>
                                <?php if($post->pin): ?>
                                    <button type="button" data-action="<?php echo e(url($topic->getPostsUrl())); ?>/<?php echo e($post->id); ?>/un_pin" class="js-btn-post-un-pin btn-transparent font-14 font-weight-500 text-warning mr-25">Unpin</button>
                                <?php else: ?>
                                    <button type="button" data-action="<?php echo e(url($topic->getPostsUrl())); ?>/<?php echo e($post->id); ?>/pin" class="js-btn-post-pin btn-transparent font-14 font-weight-500 text-gray mr-25">Pin</button>
                                <?php endif; ?>
                            <?php endif; ?>

                            <?php if(!empty($post)): ?>
                                <button type="button" data-id="<?php echo e($post->id); ?>" class="js-reply-post-btn btn-transparent mr-25 font-14 font-weight-500 text-gray">Balas</button>
                            <?php endif; ?>

                            <button type="button" data-id="<?php echo e(!empty($post) ? $post->id : $topic->id); ?>" data-type="<?php echo e(!empty($post) ? 'topic_post' : 'topic'); ?>" class="js-topic-post-report btn-transparent mr-25 font-14 font-weight-500 text-gray">Laporkan</button>
                        <?php endif; ?>

                        <div class="topic-post-like-btn d-flex align-items-center">
                            <button type="button" class="<?php echo e(!empty($authUser) ? 'js-topic-post-like' : 'login-to-access'); ?> badge-icon d-flex align-items-center justify-content-center <?php echo e(((!empty($post) and in_array($post->id,$likedPostsIds)) or (empty($post) and $topic->liked)) ? 'liked' : ''); ?>" data-action="<?php echo e(url(!empty($post) ? $post->getLikeUrl($forum->slug,$topic->slug) : $topic->getLikeUrl($forum->slug))); ?>">
...
<?php /**PATH C:\laragon\www\simpelnakes\resources\views/web/default/forum/post_card.blade.php ENDPATH**/ ?>