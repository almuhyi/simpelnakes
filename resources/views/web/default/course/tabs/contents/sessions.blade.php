    ...
    <div class="d-flex align-items-center justify-content-between" role="tab" id="session_{{ $session->id }}">
        <div class="d-flex align-items-center" href="#collapseSession{{ $session->id }}" aria-controls="collapseSession{{ $session->id }}" data-parent="#{{ $accordionParent }}" role="button" data-toggle="collapse" aria-expanded="true">
            @if($session->date > time())
                <a href="{{ url($session->addToCalendarLink()) }}" target="_blank" class="mr-15 d-flex" data-toggle="tooltip" data-placement="top" title="Tambahkan ke Kalender">
        ...
        <div class="panel-collapse">
            ...
            @if(!empty($user) and $hasBought)
                <div class="d-flex align-items-center mt-20">
                    <label class="mb-0 mr-10 cursor-pointer font-weight-500" for="sessionReadToggle{{ $session->id }}">Saya lulus pelajaran ini</label>
            ...
            <div class="d-flex align-items-center justify-content-between mt-20">
                <div class="d-flex align-items-center">
                    <div class="d-flex align-items-center text-gray text-center font-14 mr-20">
                        ...
                        <span class="line-height-1">{{ convertMinutesToHourAndMinute($session->duration) }} Jam</span>
                    </div>
                ...
                <div class="">
                    ...
                    @elseif($hasBought)
                        ...
                        @else
                            <a href="{{ url($course->getLearningPageUrl()) }}?type=session&item={{ $session->id }}" target="_blank" class="course-content-btns btn btn-sm btn-primary flex-grow-1">Pergi ke pelatihan</a>
                        @endif
                    @else
