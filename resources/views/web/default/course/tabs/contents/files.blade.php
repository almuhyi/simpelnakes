        ...
        <div class="panel-collapse">
            ...
            @if(!empty($user) and $hasBought)
                <div class="d-flex align-items-center mt-20">
                    <label class="mb-0 mr-10 cursor-pointer font-weight-500" for="fileReadToggle{{ $file->id }}">Saya lulus pelajaran ini</label>
                    ...
                    @elseif($file->accessibility == 'paid')
                        @if(!empty($user) and $hasBought)
                            @if($file->downloadable)
                                <a href="{{ url($course->getUrl()) }}/file/{{ $file->id }}/download" class="course-content-btns btn btn-sm btn-primary">
                                    Unduh
                                </a>
                            @else
                                <a href="{{ url($course->getLearningPageUrl()) }}?type=file&item={{ $file->id }}" target="_blank" class="course-content-btns btn btn-sm btn-primary">
                                    Play
                                </a>
                            @endif
                        @else
                    ...
                    @else
                        ...
                        @else
                            ...
                            @elseif($file->storage == 'upload_archive')
                                <a href="{{ url('') }}/course/{{ $course->slug }}/file/{{ $file->id }}/showHtml" target="_blank" class="course-content-btns btn btn-sm btn-primary">
                                    Play
                                </a>
                            @elseif(in_array($file->storage, ['iframe', 'google_drive', 'dropbox']))
                                <a href="{{ url('') }}/course/{{ $course->slug }}/file/{{ $file->id }}/play" target="_blank" class="course-content-btns btn btn-sm btn-primary">
                                    Play
                                </a>
                            @elseif($file->isVideo())
                            ...
                            @else
                                <a href="{{ url($file->file) }}" target="_blank" class="course-content-btns btn btn-sm btn-primary">
                                    Play
                                </a>
                            @endif
                        @endif
