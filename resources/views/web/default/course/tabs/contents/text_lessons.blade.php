        ...
        <div class="d-flex align-items-center" href="#collapseTextLessons{{ $textLesson->id }}" aria-controls="collapseTextLessons{{ $textLesson->id }}" data-parent="#{{ $accordionParent }}" role="button" data-toggle="collapse" aria-expanded="true">
            ...
            @if($textLesson->accessibility == 'paid')
                @if(!empty($user) and $hasBought)
                    <a href="{{ url($course->getLearningPageUrl()) }}?type=text_lesson&item={{ $textLesson->id }}" target="_blank" class="mr-15" data-toggle="tooltip" data-placement="top" title="Baca">
            ...
            @else
                ...
                @else
                    <a href="{{ url($course->getUrl()) }}/lessons/{{ $textLesson->id }}/read" target="_blank" class="mr-15" data-toggle="tooltip" data-placement="top" title="Baca">
                        <span class="chapter-icon chapter-content-icon">
                            <i data-feather="file-text" width="20" height="20" class="text-gray"></i>
                        </span>
                    </a>
        ...
        <div class="panel-collapse">
            ...
            @if(!empty($user) and $hasBought)
                <div class="d-flex align-items-center mt-20">
                    <label class="mb-0 mr-10 cursor-pointer font-weight-500" for="textLessonReadToggle{{ $textLesson->id }}">Saya lulus pelajaran ini</label>
            ...
            <div class="d-flex align-items-center justify-content-between mt-20">
                ...
                <div class="d-flex align-items-center">
                    <div class="d-flex align-items-center text-gray text-center font-14 mr-20">
                        ...
                        <span class="line-height-1">{{ $textLesson->study_time }} Menit</span>
                    </div>

                    <div class="d-flex align-items-center text-gray text-center font-14 mr-20">
                        <i data-feather="paperclip" width="18" height="18" class="text-gray mr-5"></i>
                        <span class="line-height-1">Lampiran file: {{ $textLesson->attachments_count }}</span>
                    </div>
                </div>
                ...
                <div class="">
                    ...
                    @elseif($textLesson->accessibility == 'paid')
                        @if(!empty($user) and $hasBought)
                            <a href="{{ url($course->getLearningPageUrl()) }}?type=text_lesson&item={{ $textLesson->id }}" target="_blank" class="course-content-btns btn btn-sm btn-primary">
                                Baca
                            </a>
                        @else
                    ...
                    @else
                        ...
                        @else
                            <a href="{{ url($course->getUrl()) }}/lessons/{{ $textLesson->id }}/read" target="_blank" class="course-content-btns btn btn-sm btn-primary">
                                Baca
                            </a>
                        @endif
                    @endif
