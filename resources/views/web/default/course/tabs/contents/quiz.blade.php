        ...
        <div class="panel-collapse">
            <div class="d-flex align-items-center mt-20">
                <div class="d-flex align-items-center text-gray text-center font-14 mr-20">
                    ...
                    <span class="line-height-1">{{ $quiz->quizQuestions->count() }} Pertanyaan</span>
                </div>

                <div class="d-flex align-items-center text-gray text-center font-14 mr-20">
                    <i data-feather="clock" width="18" height="18" class="text-gray mr-5"></i>
                    <span class="line-height-1">{{ $quiz->time }} Menit</span>
                </div>

                <div class="d-flex align-items-center text-gray text-center font-14 mr-20">
                    <i data-feather="check-square" width="18" height="18" class="text-gray mr-5"></i>
                    <span class="line-height-1">Nilai lulus: {{ $quiz->pass_mark }}/{{ $quiz->quizQuestions->sum('grade') }}</span>
                </div>

                <div class="d-flex align-items-center text-gray text-center font-14 mr-20">
                    <i data-feather="check-square" width="18" height="18" class="text-gray mr-5"></i>
                    <span class="line-height-1">Percobaan: {{ (!empty($user) and !empty($quiz->result_count)) ? $quiz->result_count : '0' }}/{{ $quiz->attempt }}</span>
                </div>
            ...
            <div class="d-flex justify-content-end mt-20">
                @if(!empty($user) and $quiz->can_try and $hasBought)
                    <a href="{{ url('') }}/panel/quizzes/{{ $quiz->id }}/start" class="course-content-btns btn btn-sm btn-primary">Mulai kuis</a>
                @else
