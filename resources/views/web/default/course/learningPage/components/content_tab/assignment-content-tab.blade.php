...
<a href="{{ url((!empty($checkSequenceContent) and $sequenceContentHasError) ? '#!' : $assignmentUrl) }}" target="{{ $assignmentUrlTarget }}" class=" d-flex align-items-start p-10 cursor-pointer {{ (!empty($checkSequenceContent) and $sequenceContentHasError) ? 'js-sequence-content-error-modal' : 'tab-item' }} {{ $user->isAdmin() ? 'js-not-access-toast' : '' }}"
    ...
    <div>
        <div class="">
            ...
            @if(empty($itemHistory) or ($itemHistory->status == \App\Models\WebinarAssignmentHistory::$notSubmitted))
                <span class="text-danger font-12 d-block">Tugas belum dikumpulkan</span>
            @else
                @switch($itemHistory->status)
                    @case(\App\Models\WebinarAssignmentHistory::$passed)
                    <span class="text-primary font-12 d-block">Lulus</span>
                    @break
                    @case(\App\Models\WebinarAssignmentHistory::$pending)
                    <span class="text-warning font-12 d-block">Tertunda</span>
                    @break
                    @case(\App\Models\WebinarAssignmentHistory::$notPassed)
                    <span class="font-12 d-block text-danger">Gagal</span>
                    @break
                @endswitch
            @endif
        </div>
