                ...
                <div class="">
                    ...
                    @if(empty($answer) and !empty($courseForum->attach))
                        <div class="mt-25 d-inline-block">
                            <a href="{{ url($course->getForumPageUrl()) }}/{{ $courseForum->id }}/downloadAttach" target="_blank" class="d-flex align-items-center text-gray bg-info-light border px-10 py-5 rounded-pill">
                ...
                <div class="d-flex align-items-center justify-content-between mt-15 pt-15 border-top">
                    ...
                    <div class="d-flex align-items-center">
                        @if(empty($answer) and $user->id == $courseForum->user_id)
                            <button type="button" data-action="{{ url($course->getForumPageUrl()) }}/{{ $courseForum->id }}/edit" class="js-edit-forum btn-transparent font-12 font-weight-500 text-gray">Edit</button>
                        @elseif(!empty($answer))
                            @if($course->isOwner($user->id))
                                @if($answer->pin)
                                    <button type="button" data-action="{{ url($course->getForumPageUrl()) }}/{{ $courseForum->id }}/answers/{{ $answer->id }}/un_pin" class="js-btn-answer-un_pin btn-transparent font-12 font-weight-500 text-warning">Unpin</button>
                                @else
                                    <button type="button" data-action="{{ url($course->getForumPageUrl()) }}/{{ $courseForum->id }}/answers/{{ $answer->id }}/pin" class="js-btn-answer-pin btn-transparent font-12 font-weight-500 text-gray">Pin</button>
                                @endif
                            @endif

                            @if($course->isOwner($user->id) or $user->id == $courseForum->user_id)
                                @if($answer->resolved)
                                    <button type="button" data-action="{{ url($course->getForumPageUrl()) }}/{{ $courseForum->id }}/answers/{{ $answer->id }}/mark_as_not_resolved" class="js-btn-answer-mark_as_not_resolved btn-transparent font-12 font-weight-500 text-gray ml-20">Tandai sebagai belum terselesaikan</button>
                                @else
                                    <button type="button" data-action="{{ url($course->getForumPageUrl()) }}/{{ $courseForum->id }}/answers/{{ $answer->id }}/mark_as_resolved" class="js-btn-answer-mark_as_resolved btn-transparent font-12 font-weight-500 text-gray ml-20">Tandai sebagai terselesaikan</button>
                                @endif
                            @endif

                            @if($user->id == $answer->user_id)
                                <button type="button" data-action="{{ url($course->getForumPageUrl()) }}/{{ $courseForum->id }}/answers/{{ $answer->id }}/edit" class="js-edit-forum-answer btn-transparent font-12 font-weight-500 text-gray ml-20">Edit</button>
                            @endif
