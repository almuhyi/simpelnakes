...
@section('content')
    <section class="course-cover-container {{ empty($activeSpecialOffer) ? 'not-active-special-offer' : '' }}">
        <img src="{{ asset($course->getImageCover()) }}" class="img-cover course-cover-img" alt="{{ $course->title }}"/>
                ...
                <div class="course-content-body user-select-none">
                    <div class="course-body-on-cover text-white">
                        ...
                        @if(!empty($course->category))
                            <span class="d-block font-16 mt-10">Kategori <a href="{{ url($course->category->getUrl()) }}" target="_blank" class="font-weight-500 text-decoration-underline text-white">{{ $course->category->title }}</a></span>
                        @endif

                        <div class="d-flex align-items-center">
                            @include('web.default.includes.webinar.rate',['rate' => $course->getRate()])
                            <span class="ml-10 mt-15 font-14">({{ $course->reviews->pluck('creator_id')->count() }} Ulasan)</span>
                        </div>

                        <div class="mt-15">
                            <span class="font-14">Dibuat oleh</span>
                            <a href="{{ url($course->teacher->getProfileUrl()) }}" target="_blank" class="text-decoration-underline text-white font-14 font-weight-500">{{ $course->teacher->full_name }}</a>
                        </div>
                                ...
                                <span class="ml-15 font-14 font-weight-500">
                                    @if($course->isWebinar())
                                        @if($hasBought and $course->isProgressing())
                                            {{ $percent }}% dari pelatihan lulus.
                                        @else
                                            {{ $course->sales_count }}/{{ $course->capacity }} Peserta
                                        @endif
                                    @else
                                        {{ $percent }}% dari pelatihan lulus.
                                    @endif
                        ...
                        <ul class="nav nav-tabs bg-secondary rounded-sm p-15 d-flex align-items-center justify-content-between" id="tabs-tab" role="tablist">
                            ...
                            <li class="nav-item">
                                <a class="position-relative font-14 text-white {{ (request()->get('tab','') == 'content') ? 'active' : '' }}" id="content-tab" data-toggle="tab"
                                   ...
                                   aria-selected="false">Materi ({{ $webinarContentCount }})</a>
                            </li>
                            <li class="nav-item">
                                <a class="position-relative font-14 text-white {{ (request()->get('tab','') == 'reviews') ? 'active' : '' }}" id="reviews-tab" data-toggle="tab"
                                   href="#reviews" role="tab" aria-controls="reviews"
                                   aria-selected="false">Ulasan ({{ $course->reviews->count() > 0 ? $course->reviews->pluck('creator_id')->count() : 0 }})</a>
                            </li>
                        </ul>
            ...
            <div class="course-content-sidebar col-12 col-lg-4 mt-25 mt-lg-0">
                <div class="rounded-lg shadow-sm">
                    <div class="course-img {{ $course->video_demo ? 'has-video' :'' }}">
                        ...
                        <img src="{{ asset($course->getImage()) }}" class="img-cover" alt="">
                    ...
                    <div class="px-20 pb-30">
                        <form action="{{ url('/cart/store') }}" method="post">
                            ...
                            <div class="mt-20 d-flex flex-column">
                                @if($hasBought)
                                    <a href="{{ url($course->getLearningPageUrl()) }}" class="btn btn-primary">Pergi ke halaman pelatihan</a>
                                @elseif($course->price > 0)
                                    ...
                                    @if($canSale and $course->subscribe)
                                        <a href="{{ url('') }}/subscribes/apply/{{ $course->slug }}" class="btn btn-outline-primary btn-subscribe mt-20 @if(!$canSale) disabled @endif">
                                            Langganan</a>
                                    @endif

                                    @if($canSale and !empty($course->points))
                                        <a href="{{ url(!(auth()->check()) ? '/login' : '#') }}" class="{{ (auth()->check()) ? 'js-buy-with-point' : '' }} btn btn-outline-warning mt-20 {{ (!$canSale) ? 'disabled' : '' }}" rel="nofollow">
                                            Beli dengan {!! $course->points !!} point
                                        </a>
                                    @endif
                                    ...
                                    @endif
                                @else
                                    <a href="{{ url($canSale ? '/course/'. $course->slug .'/free' : '#') }}" class="btn btn-primary {{ (!$canSale) ? (' disabled ' . $course->cantSaleStatus($hasBought)) : '' }}">Daftar di pelatihan</a>
                                @endif
                            </div>
                        ...
                        <div class="mt-35">
                            <strong class="d-block text-secondary font-weight-bold">{{ $course->type }} ini meliputi</strong>
                            ...
                            @if($course->quizzes->where('status', \App\models\Quiz::ACTIVE)->count() > 0)
                                <div class="mt-20 d-flex align-items-center text-gray">
                                    ...
                                    <span class="ml-5 font-14 font-weight-500">{{ $course->quizzes->where('status', \App\models\Quiz::ACTIVE)->count() }} Kuis Online</span>
                                </div>
                            @endif
                        ...
                        <div class="mt-40 p-10 rounded-sm border row align-items-center favorites-share-box">
                            @if($course->isWebinar())
                                <div class="col">
                                    <a href="{{ url($course->addToCalendarLink()) }}" target="_blank" class="d-flex flex-column align-items-center text-center text-gray">
                            ...
                            <div class="col">
                                <a href="{{ url('') }}/favorites/{{ $course->slug }}/toggle" id="favoriteToggle" class="d-flex flex-column align-items-center text-gray">
                ...
                @if($course->teacher->offline)
                    <div class="rounded-lg shadow-sm mt-35 d-flex">
                        <div class="offline-icon offline-icon-left d-flex align-items-stretch">
                            <div class="d-flex align-items-center">
                                <img src="{{ asset('') }}assets/default/img/profile/time-icon.png" alt="offline">
                            </div>
                        </div>
                ...
                <div class="rounded-lg shadow-sm mt-35 px-25 py-20">
                    <h3 class="sidebar-title font-16 text-secondary font-weight-bold">{{ $course->type .' '. ('Spesifikasi') }}</h3>
                    ...
                    <div class="mt-30">
                        @if($course->isWebinar())
                            ...
                            <div class="mt-20 d-flex align-items-center justify-content-between text-gray">
                                ...
                                <span class="font-14">{{ $course->capacity }} Peserta</span>
                            </div>
                        @endif
                        ...
                        <div class="mt-20 d-flex align-items-center justify-content-between text-gray">
                            ...
                            <span class="font-14">{{ convertMinutesToHourAndMinute(!empty($course->duration) ? $course->duration : 0) }} Jam</span>
                        </div>
                        ...
                        @if($course->isWebinar())
                            <div class="mt-20 d-flex align-items-center justify-content-between text-gray">
                                <div class="d-flex align-items-center">
                                    <img src="{{ asset('') }}assets/default/img/icons/sessions.svg" width="20" alt="">
                        ...
                        @if(!empty($course->access_days))
                            <div class="mt-20 d-flex align-items-center justify-content-between text-gray">
                                ...
                                <span class="font-14">{{ $course->access_days }} Hari</span>
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="row">
                        @foreach($advertisingBannersSidebar as $sidebarBanner)
                            <div class="rounded-lg sidebar-ads mt-35 col-{{ $sidebarBanner->size }}">
                                <a href="{{ url($sidebarBanner->link) }}">
                                    <img src="{{ asset($sidebarBanner->image) }}" class="img-cover rounded-lg" alt="{{ $sidebarBanner->title }}">
                                </a>
                            </div>
                        @endforeach
                    </div>
                    ...
                    @foreach($advertisingBanners as $banner)
                        <div class="col-{{ $banner->size }}">
                            <a href="{{ url($banner->link) }}">
                                <img src="{{ asset($banner->image) }}" class="img-cover rounded-sm" alt="{{ $banner->title }}">
                            </a>
                        </div>
                    @endforeach
                </div>
    ...
    <div id="webinarReportModal" class="d-none">
        ...
        <form action="{{ url('') }}/course/{{ $course->id }}/report" method="post" class="mt-25">
...
@push('scripts_bottom')
    ...
    <script src="{{ asset('') }}assets/default/vendors/barrating/jquery.barrating.min.js"></script>
    ...
    <script>
        ...
        var notAccessToastMsgLang = '{{ ('Anda tidak memiliki akses ke materi ini.') }}';
        ...
        var courseHasBoughtStatusToastTitleLang = '{{ ('Pembelian gagal!') }}';
        var courseHasBoughtStatusToastMsgLang = '{{ ('Pelatihan berhasil didaftarkan.') }}';
        var courseNotCapacityStatusToastTitleLang = '{{ ('Permintaan gagal') }}';
        var courseNotCapacityStatusToastMsgLang = '{{ ('Di luar kapasitas kelas!') }}';
        var courseHasStartedStatusToastTitleLang = '{{ ('Pembelian gagal!') }}';
        var courseHasStartedStatusToastMsgLang = '{{ ('Kelas telah dimulai') }}';
    ...
    <script src="{{ asset('') }}assets/default/js/parts/webinar_show.min.js"></script>
@endpush
