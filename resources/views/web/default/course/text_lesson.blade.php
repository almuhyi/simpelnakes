            ...
            <div class="row h-100 align-items-center justify-content-center text-center">
                <div class="col-12 col-md-9 col-lg-7">
                    ...
                    <div class="mt-20 font-16 font-weight-500 text-white">
                        <span>Materi {{ $textLesson->order }}/{{ count($course->textLessons) }} </span> | <span>Waktu belajar: {{ $textLesson->study_time }} Menit</span>
                    </div>

                    <div class="mt-20 font-16 font-weight-500 text-white">
                        <span>Pelatihan: <a href="{{ url($course->getUrl()) }}" class="font-16 font-weight-500 text-white text-decoration-underline">{{ $course->title }}</a></span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="col-12 col-lg-8">
                <div class="post-show mt-30">
                    ...
                    <div class="post-img pb-30">
                        <img src="{{ asset($textLesson->image) }}" alt="{{ $textLesson->title }}"/>
                    </div>
                ...
                <div class="mt-30 row align-items-center">
                    ...
                    <div class="col-12 col-md-7 text-right">
                        @if(!empty($course->textLessons) and count($course->textLessons))
                            <a href="{{ url((!empty($previousLesson)) ? $course->getUrl() .'/lessons/'. $previousLesson->id .'/read' : '#') }}" class="btn btn-sm {{ (!empty($previousLesson)) ? 'btn-primary' : 'btn-gray disabled' }}">Materi sebelumnya</a>

                            @if(!empty($nextLesson))
                                <a href="{{ url((!$nextLesson->not_purchased) ? $course->getUrl() .'/lessons/'. $nextLesson->id .'/read' : '#') }}" class="btn btn-sm {{ (!$nextLesson->not_purchased) ? 'btn-primary' : 'btn-gray disabled' }} {{ ($nextLesson->not_purchased) ? 'js-not-purchased' : '' }}">Materi selanjutnya</a>
                            @endif
                        @endif
                    </div>
                </div>
            ...
            <div class="col-12 col-lg-4">
                ...
                <div class="rounded-lg shadow-sm mt-35 p-20 course-teacher-card d-flex align-items-center flex-column">
                    <div class="teacher-avatar mt-5">
                        <img src="{{ asset($course->teacher->getAvatar(100)) }}" class="img-cover" alt="{{ $course->teacher->full_name }}">
                    </div>
                    ...
                    <div class="user-reward-badges d-flex flex-wrap align-items-center mt-20">
                        @foreach($course->teacher->getBadges() as $userBadge)
                            <div class="mr-15 mt-10" data-toggle="tooltip" data-placement="bottom" data-html="true" title="{!! (!empty($userBadge->badge_id) ? nl2br($userBadge->badge->description) : nl2br($userBadge->description)) !!}">
                                <img src="{{ url(!empty($userBadge->badge_id) ? $userBadge->badge->image : $userBadge->image) }}" width="32" height="32" alt="{{ !empty($userBadge->badge_id) ? $userBadge->badge->title : $userBadge->title }}">
                            </div>
                        @endforeach
                    </div>

                    <div class="mt-25 d-flex flex-row align-items-center justify-content-center w-100">
                        <a href="{{ $course->teacher->getProfileUrl() }}" target="_blank" class="btn btn-sm btn-primary teacher-btn-action">Profil</a>

                        @if(!empty($course->teacher->hasMeeting()))
                            <a href="{{ url($course->teacher->getProfileUrl()) }}" class="btn btn-sm btn-primary teacher-btn-action ml-15">Ajukan pertemuan</a>
                        @else
                        ...
                        <ul class="p-0 m-0 pt-10">
                            @foreach($textLesson->attachments as $attachment)
                                <li class="mt-10 p-10 rounded bg-info-light font-14 font-weight-500 text-dark-blue d-flex align-items-center justify-content-between text-ellipsis">
                                    ...
                                    <a href="{{ url($course->getLearningPageUrl()) }}?type=file&item={{ $attachment->file->id }}" target="_blank">
                    ...
                    <div class="shadow-sm rounded-lg bg-white px-15 px-md-25 py-20 mt-30">
                        ...
                        <div class="p-0 m-0 pt-10">
                            @foreach($course->textLessons as $lesson)
                                <a href="{{ url($course->getUrl()) }}/lessons/{{ $lesson->id }}/read"
...
@push('scripts_bottom')
    ...
    <script src="{{ asset('') }}assets/default/js/parts/text_lesson.min.js"></script>
@endpush
