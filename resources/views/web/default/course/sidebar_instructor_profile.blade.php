<div class="rounded-lg shadow-sm mt-35 p-20 course-teacher-card d-flex align-items-center flex-column">
    ...
    <div class="teacher-avatar mt-5">
        <img src="{{ asset($courseTeacher->getAvatar(100)) }}" class="img-cover" alt="{{ $courseTeacher->full_name }}">
    ...
    <div class="mt-25 d-flex flex-row align-items-center justify-content-center w-100">
        <a href="{{ url($courseTeacher->getProfileUrl()) }}" target="_blank" class="btn btn-sm btn-primary {{ $hasMeeting ? 'teacher-btn-action' : 'btn-block' }}">Profil</a>

        @if($hasMeeting)
            <a href="{{ url($courseTeacher->getProfileUrl()) }}" class="btn btn-sm btn-primary teacher-btn-action ml-15">Ajukan pertemuan</a>
        @endif
    </div>
</div>
