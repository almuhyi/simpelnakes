...
@section('content')
    <div class="container">
        <section class="topics-title-section mt-30 mt-md-50 px-20 px-md-30 py-25 py-md-35 rounded-lg">
            <h1 class="font-30 font-weight-bold text-white">{{ !empty($topic) ? ('Edit topik') : ('Topik baru') }}</h1>
            ...
            <div class="mt-10">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb p-0 m-0">
                        ...
                        <li class="breadcrumb-item font-12 text-white font-weight-bold" aria-current="page">{{ !empty($topic) ? ('Edit topik') : ('Topik baru') }}</li>
                    </ol>
                </nav>
            </div>
        </section>
                ...
                <div class="row">
                    <div class="col-12 col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="title" value="{{ !empty($topic) ? $topic->title : old('title') }}" class="form-control @error('title') is-invalid @enderror" placeholder="Judul topik">
                            @error('title')
                            ...
                            @if(!empty($topic) and !empty($topic->attachments) and count($topic->attachments))
                                @foreach($topic->attachments as $topicAttachment)
                                    <div class="input-group product-images-input-group mt-10">
                                        ...
                                        <input type="text" name="attachments[]" id="attachments_{{ $topicAttachment->id }}" value="{{ $topicAttachment->path }}" class="form-control" placeholder="Ukuran lampiran file"/>
