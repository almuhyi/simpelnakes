...
@section('content')
    <div class="container">
        <section class="topics-title-section mt-30 mt-md-50 px-20 px-md-30 py-25 py-md-35 rounded-lg" style="background-image: url('{{asset('/assets/default/img/forum/topics_title_bg.png')}}')">
        ...
        <div class="topics-filters-section bg-white rounded-lg px-20 py-25 mt-40">
            <div class="row">
                <div class="col-12 col-md-5">
                    <h3 class="font-16 font-weight-bold text-secondary">{{ $topics->count() }} topik diskusi di forum ini</h3>

                    <div class="d-flex align-items-center mt-5">
                        <a href="{{ request()->url() }}?sort=newest" class="font-14 font-weight-500 mr-20 {{ (empty(request()->get('sort')) or request()->get('sort') == 'newest') ? 'text-primary' : 'text-gray' }}">Terbaru</a>
                        <a href="{{ request()->url() }}?sort=popular_topics" class="font-14 font-weight-500 mr-20 {{ (request()->get('sort') == 'popular_topics') ? 'text-primary' : 'text-gray' }}">Topik diskusi populer</a>
                        <a href="{{ request()->url() }}?sort=not_answered" class="font-14 font-weight-500 {{ (request()->get('sort') == 'not_answered') ? 'text-primary' : 'text-gray' }}">Tidak dijawab</a>
                    </div>
                </div>
                ...
                <div class="col-12 col-md-7  mt-15 mt-lg-0">
                    <div class="row">
                        <div class="col-12 col-lg-7">
                            <form action="" method="get">
                                <div class="d-flex align-items-center">
                                    <input type="text" name="search" value="{{ request()->get('search') }}" class="form-control flex-grow-1 input-search-topic" placeholder="Cari di forum diskusi ini">
                        ...
                        <div class="col-12 col-lg-5 mt-15 mt-lg-0 text-right">
                            ...
                            @else
                                <a href="{{ url('') }}/forums/create-topic{{ $forum->checkUserCanCreateTopic() ? '?forum_id='.$forum->id : '' }}" class="btn btn-primary btn-create-topic btn-block">
                                    <i data-feather="file" class="text-white" width="16" height="16"></i>
                                        ...
                                        <div class="col-12 col-md-6">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div class="ml-10 mw-100">
                                                    ...
                                                    <span class="d-block font-12 mt-5 text-gray">Oleh {{ $topic->creator->full_name }} Pada {{ dateTimeFormat($topic->created_at,'j M Y | H:i') }}</span>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    @if(!empty($popularTopics) and count($popularTopics))
                        <div class="rounded-lg p-15 border bg-white mt-20">
                            ...
                            @foreach($popularTopics as $popularTopic)
                                <div class="d-flex align-items-center mt-15">
                                    <div class="topics-right-side-user-avatar rounded-circle">
                                        <img src="{{ asset(!empty($popularTopic->creator) ? $popularTopic->creator->getAvatar(48) : '') }}" class="img-cover rounded-circle" alt="{{ !empty($popularTopic->creator) ? $popularTopic->creator->full_name : '' }}">
                                    </div>
                                    <div class="ml-10">
                                        <a href="{{ url($popularTopic->getPostsUrl()) }}" class="d-block pb-5">
