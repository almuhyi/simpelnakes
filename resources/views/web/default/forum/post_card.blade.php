    ...
    <div class="d-flex flex-wrap">
        <div class="col-12 col-md-3">
            <div class="position-relative bg-info-light d-flex flex-column align-items-center justify-content-start rounded-lg w-100 h-100 p-20">
                ...
                <a href="{{ url($cardUser->getProfileUrl()) }}" target="_blank">
                    ...
                    <div class="d-flex flex-wrap align-items-center justify-content-center mt-20 w-100">
                        @foreach($cardUserBadges as $badge)
                            <div class="mr-10 mt-10" data-toggle="tooltip" data-placement="bottom" data-html="true" title="{!! (!empty($badge->badge_id) ? nl2br($badge->badge->description) : nl2br($badge->description)) !!}">
                                <img src="{{ url(!empty($badge->badge_id) ? $badge->badge->image : $badge->image) }}" width="32" height="32" alt="{{ !empty($badge->badge_id) ? $badge->badge->title : $badge->title }}">
                            </div>
                        @endforeach
                    </div>
                @endif
                ...
                <div class="d-flex flex-column h-100">
                    ...
                    @if(!empty($post) and !empty($post->attach))
                        <div class="mt-auto d-inline-flex">
                            <a href="{{ url($post->getAttachmentUrl($forum->slug,$topic->slug)) }}" target="_blank" class="d-flex align-items-center text-gray bg-info-light border px-10 py-5 rounded-pill">
                    ...
                    @elseif(empty($post) and !empty($topic->attachments) and count($topic->attachments))
                        <div class="mt-auto d-inline-flex align-items-center">
                            @foreach($topic->attachments as $attachment)
                                <a href="{{ url($attachment->getDownloadUrl($forum->slug,$topic->slug)) }}" target="_blank" class="d-flex align-items-center text-gray bg-info-light border px-10 py-5 rounded-pill mr-15">
                                    <i data-feather="paperclip" class="text-gray" width="16" height="16"></i>
                    ...
                    <div class="d-flex align-items-center">
                        @if(!empty($authUser) and !$topic->close)
                            @if($authUser->id == $cardUser->id)
                                @if(!empty($post))
                                    <button type="button" data-action="{{ url($post->getEditUrl($forum->slug,$topic->slug)) }}" class="js-post-edit btn-transparent mr-25 font-14 font-weight-500 text-gray">Edit</button>
                                @else
                                    <a href="{{ url($topic->getEditUrl($forum->slug)) }}" class="mr-25 font-14 font-weight-500 text-gray">Edit</a>
                                @endif
                            @endif

                            @if(!empty($post) and $authUser->id == $topic->creator_id)
                                @if($post->pin)
                                    <button type="button" data-action="{{ url($topic->getPostsUrl()) }}/{{ $post->id }}/un_pin" class="js-btn-post-un-pin btn-transparent font-14 font-weight-500 text-warning mr-25">Unpin</button>
                                @else
                                    <button type="button" data-action="{{ url($topic->getPostsUrl()) }}/{{ $post->id }}/pin" class="js-btn-post-pin btn-transparent font-14 font-weight-500 text-gray mr-25">Pin</button>
                                @endif
                            @endif

                            @if(!empty($post))
                                <button type="button" data-id="{{ $post->id }}" class="js-reply-post-btn btn-transparent mr-25 font-14 font-weight-500 text-gray">Balas</button>
                            @endif

                            <button type="button" data-id="{{ !empty($post) ? $post->id : $topic->id }}" data-type="{{ !empty($post) ? 'topic_post' : 'topic' }}" class="js-topic-post-report btn-transparent mr-25 font-14 font-weight-500 text-gray">Laporkan</button>
                        @endif

                        <div class="topic-post-like-btn d-flex align-items-center">
                            <button type="button" class="{{ !empty($authUser) ? 'js-topic-post-like' : 'login-to-access' }} badge-icon d-flex align-items-center justify-content-center {{ ((!empty($post) and in_array($post->id,$likedPostsIds)) or (empty($post) and $topic->liked)) ? 'liked' : '' }}" data-action="{{ url(!empty($post) ? $post->getLikeUrl($forum->slug,$topic->slug) : $topic->getLikeUrl($forum->slug)) }}">
