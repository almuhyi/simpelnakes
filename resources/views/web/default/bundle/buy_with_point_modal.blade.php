    ...
    @if(!empty($user))
        <div class="text-center">
            ...
            <p class="font-14 font-weight-500 text-gray mt-30">
                <span class="d-block">Pelatihan ini membutuhkan{{ $bundle->points }} poin</span>
        ...
        <div class="d-flex align-items-center mt-25">
            <a href="{{ ($user->getRewardPoints() >= $bundle->points) ? '/bundles/'. $bundle->slug .'/points/apply' : '#' }}" class="btn btn-sm flex-grow-1 {{ ($user->getRewardPoints() >= $bundle->points) ? 'btn-primary js-buy-course-with-point' : 'bg-gray300 text-gray disabled' }}">Beli</a>
