...
@section('content')
    <section class="site-top-banner search-top-banner opacity-04 position-relative">
        <img src="{{ asset(getPageBackgroundSettings($page)) }}" class="img-cover" alt=""/>
                        ...
                        <div class="search-input bg-white p-10 flex-grow-1">
                            <form action="{{ url('/{{ $page }}') }}" method="get">
                                <div class="form-group d-flex align-items-center m-0">
                                    <input type="text" name="search" class="form-control border-0" value="{{ request()->get('search') }}" placeholder="Cari {{ $title }}"/>
    ...
    <div class="container">
        ...
        <form id="filtersForm" action="{{ url('/{{ $page }}') }}" method="get">
                ...
                <div class="row align-items-center">
                    ...
                    <div class="col-lg-3 d-flex align-items-center">
                        <select name="sort" class="form-control">
                            ...
                            <option value="top_rate" @if(request()->get('sort',null) == 'top_rate') selected="selected" @endif>Top rate</option>
        ...
        <section>
            ...
            <div class="text-center">
                <button type="button" id="loadMoreInstructors" data-page="{{ ($page == 'instructors') ? \App\Models\Role::$teacher : \App\Models\Role::$organization }}" class="btn btn-border-white mt-50 {{ ($instructors->lastPage() <= $instructors->currentPage()) ? ' d-none' : '' }}">Muat instruktur lainnya</button>
            </div>
        </section>
        ...
        @if(!empty($bestRateInstructors) and !$bestRateInstructors->isEmpty() and (empty(request()->get('sort')) or !in_array(request()->get('sort'),['top_rate','top_sale'])))
            <section class="mt-30 pt-30">
                <div class="d-flex justify-content-between">
                    ...
                    <a href="{{ url('/{{ $page }}?sort=top_rate')}}" class="btn btn-border-white">Lihat semua</a>
                </div>
        ...
        @if(!empty($bestSalesInstructors) and !$bestSalesInstructors->isEmpty() and (empty(request()->get('sort')) or !in_array(request()->get('sort'),['top_rate','top_sale'])))
            <section class="mt-50 pt-50">
                <div class="d-flex justify-content-between">
                    ...
                    <a href="{{ url('/{{ $page }}?sort=top_sale')}}" class="btn btn-border-white">Lihat semua</a>
                </div>
