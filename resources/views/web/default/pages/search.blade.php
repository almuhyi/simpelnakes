...
@section('content')
    @if((!empty($webinars) and count($webinars)) or (!empty($products) and count($products)) or (!empty($teachers) and count($teachers)) or (!empty($organizations) and count($organizations)))
        <section class="site-top-banner search-top-banner opacity-04 position-relative">
            <img src="{{ asset(getPageBackgroundSettings('search')) }}" class="img-cover" alt=""/>

            <div class="container h-100">
                <div class="row h-100 align-items-center justify-content-center text-center">
                    <div class="col-12 col-md-9 col-lg-7">
                        <div class="top-search-form">
                            <h1 class="text-white font-30"> {{ $resultCount }} Hasil ditemukan untuk <br> "{{ request()->get('search') }}" </h1>

                            <div class="search-input bg-white p-10 flex-grow-1">
                                <form action="{{ url('/search') }}" method="get">
                                    <div class="form-group d-flex align-items-center m-0">
                                        <input type="text" name="search" class="form-control border-0" value="{{ request()->get('search','') }}" placeholder="Cari pelatihan dan instruktur"/>
                            ...
                            <div class="col-6 col-md-3 col-lg-2 mt-30">
                                <div class="user-search-card text-center d-flex flex-column align-items-center justify-content-center">
                                    <div class="user-avatar">
                                        <img src="{{ asset($teacher->getAvatar()) }}" class="img-cover rounded-circle" alt="{{ $teacher->full_name }}">
                                    </div>
                                    <a href="{{ url($teacher->getProfileUrl()) }}">
                    ...
                    <div class="row">
                        ...
                        @foreach($organizations as $organization)
                            <div class="col-md-6 col-lg-3 mt-30">
                                <a href="{{ url($organization->getProfileUrl()) }}" class="home-organizations-card d-flex flex-column align-items-center justify-content-center">
                                    <div class="home-organizations-avatar">
                                        <img src="{{ asset($organization->getAvatar()) }}" class="img-cover rounded-circle" alt="{{ $organization->full_name }}">
                                    </div>
                                    <div class="mt-25 d-flex flex-column align-items-center justify-content-center">
                                        ...
                                        <span class="home-organizations-badge badge mt-15">{{ $organization->getActiveWebinars(true) }} Pelatihan}</span>
                                    </div>
                                </a>
                            </div>
                        @endforeach
                ...
                <div class="row h-100 align-items-center justify-content-center text-center">
                    <div class="col-12 col-md-9 col-lg-7">
                        <div class="d-flex align-items-center flex-column mt-30 text-center w-100">
                            ...
                            <p class="mt-5 text-center">Tidak ada hasil yang ditemukan untuk "{{ request()->get('search') }}" Anda dapat mencoba lagi.</p>
