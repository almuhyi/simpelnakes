...
<div class="rounded-lg shadow-sm mt-25 p-20 course-teacher-card instructors-list text-center d-flex align-items-center flex-column position-relative">
    @if(!empty($instructor->meeting) and $instructor->meeting->disabled)
        <span class="px-15 py-10 bg-gray off-label text-white font-12">Tidak tersedia</span>
    @elseif(!empty($instructor->meeting) and !empty($instructor->meeting->discount))
        <span class="px-15 py-10 bg-danger off-label text-white font-12">{{ $instructor->meeting->discount }}% Off</span>
    @endif

    <a href="{{ url($instructor->getProfileUrl()) }}{{ ($canReserve) ? '?tab=appointments' : '' }}" class="text-center d-flex flex-column align-items-center justify-content-center">
        <div class=" teacher-avatar mt-5 position-relative">
            <img src="{{ asset($instructor->getAvatar(190)) }}" class="img-cover" alt="{{ $instructor->full_name }}">
    ...
    <div class="d-flex align-items-center mt-20">
        @foreach($instructor->getBadges() as $badge)
            <div class="mr-15 mt-5" data-toggle="tooltip" data-placement="bottom" data-html="true" title="{!! (!empty($badge->badge_id) ? nl2br($badge->badge->description) : nl2br($badge->description)) !!}">
                <img src="{{ asset(!empty($badge->badge_id) ? $badge->badge->image : $badge->image) }}" width="32" height="32" alt="{{ !empty($badge->badge_id) ? $badge->badge->title : $badge->title }}">
            </div>
        @endforeach
    </div>
    ...
    <div class="mt-20 d-flex flex-row align-items-center justify-content-center w-100">
        <a href="{{ url($instructor->getProfileUrl()) }}{{ ($canReserve) ? '?tab=appointments' : '' }}" class="btn btn-primary btn-block">
            @if($canReserve)
                Ajukan pertemuan
            @else
                Lihat profil
