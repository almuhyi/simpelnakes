...
@section('content')
    <section class="mt-15">
        <h2 class="section-title">Filter post</h2>

        <div class="panel-section-card py-20 px-25 mt-20">
            <form action="{{ url('/panel/forums/posts') }}" method="get" class="row">
    ...
    <section class="mt-35">
                                        ...
                                        <td class="text-left align-middle">
                                            <div class="user-inline-avatar d-flex align-items-center">
                                                <div class="avatar bg-gray200">
                                                    <img src="{{ asset($post->topic->creator->getAvatar(48)) }}" class="img-cover" alt="">
                                                </div>
                                                <a href="{{ url($post->topic->getPostsUrl()) }}" target="_blank" class="">
                                                    <div class=" ml-5">
                                                        <span class="d-block font-16 font-weight-500 text-dark-blue">{{ $post->topic->title }}</span>
                                                        <span class="font-12 text-gray mt-5">Oleh {{ $post->topic->creator->full_name }}</span>
                                                    </div>
                                                </a>
                                            </div>
                                        </td>
        ...
        @else
            ...
            @include(getTemplate() . '.includes.no-result',[
                ...
                'hint' => nl2br(('Jelajahi forum dan balas topik.')),
            ])
