...
@section('content')
    ...
    <section class="mt-25">
        ...
        <div class="panel-section-card py-20 px-25 mt-20">
            <form action="{{ url('/panel/quizzes/my-results') }}" method="get" class="row">
                                    ...
                                    <tr>
                                        <td class="text-left">
                                            <div class="user-inline-avatar d-flex align-items-center">
                                                <div class="avatar bg-gray200">
                                                    <img src="{{ asset($result->quiz->creator->getAvatar()) }}" class="img-cover" alt="">
                                                </div>
                                                <div class=" ml-5">
                                        ...
                                        <span class="d-block text-{{ ($result->status == 'passed') ? 'primary' : ($result->status == 'waiting' ? 'warning' : 'danger') }}">
                                            ...
                                            @if($result->status =='failed' and $result->can_try)
                                                <span class="d-block font-12 text-gray">{{ $result->count_can_try }} Kesempatan lagi</span>
                                            @endif
                                        </td>
                                            ...
                                            <div class="btn-group dropdown table-actions table-actions-lg table-actions-lg">
                                                ...
                                                <div class="dropdown-menu">
                                                    @if(!$result->can_try and $result->status != 'waiting')
                                                        <a href="{{ url('') }}/panel/quizzes/{{ $result->id }}/result" class="webinar-actions d-block mt-10">Lihat jawaban</a>
                                                    @endif

                                                    @if($result->status != 'passed')
                                                        @if($result->can_try)
                                                            <a href="{{ url('') }}/panel/quizzes/{{ $result->quiz->id }}/start" class="webinar-actions d-block mt-10">Coba lagi</a>
                                                        @endif
                                                    @endif

                                                    <a href="{{ url($result->quiz->webinar->getUrl()) }}" class="webinar-actions d-block mt-10">Halaman pelatihan</a>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
