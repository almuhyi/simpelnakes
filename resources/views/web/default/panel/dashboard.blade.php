...
@push('styles_top')
    ...
    <link rel="stylesheet" href="{{ asset('') }}assets/default/vendors/apexcharts/apexcharts.css"/>
@endpush
...
@section('content')
    <section class="">
        ...
        @if(!$authUser->financial_approval and !$authUser->isUser())
            <div class="p-15 mt-20 p-lg-20 not-verified-alert font-weight-500 text-dark-blue rounded-sm panel-shadow">
                ...
                <a href="{{ url('/panel/setting/step/7') }}" class="text-decoration-underline">Pengaturan</a>.
            </div>
        @endif
        ...
        <div class="bg-white dashboard-banner-container position-relative px-15 px-ld-35 py-10 panel-shadow rounded-sm">
            <h2 class="font-30 text-primary line-height-1">
                <span class="d-block">Hai {{ $authUser->full_name }},</span>
                <span class="font-16 text-secondary font-weight-bold">Kamu memiliki {{ !empty($unReadNotifications) ? count($unReadNotifications) : 0 }} notifikasi baru</span>
            </h2>
            ...
            <a href="{{ url('/panel/notifications') }}" class="mt-15 font-weight-500 text-dark-blue d-inline-block">Lihat semua notifikasi</a>

            <div class="dashboard-banner">
                <img src="{{ asset(getPageBackgroundSettings('dashboard')) }}" alt="" class="img-cover">
            </div>
        </div>
    </section>
    ...
    <section class="dashboard">
        <div class="row">
            <div class="col-12 col-lg-3 mt-35">
                <div class="bg-white account-balance rounded-sm panel-shadow py-15 py-md-30 px-10 px-md-20">
                    <div class="text-center">
                        <img src="{{ asset('') }}assets/default/img/activity/36.svg" class="account-balance-icon" alt="">
                    ...
                    <div class="mt-20 pt-30 border-top border-gray300 d-flex align-items-center @if($can_drawable) justify-content-between @else justify-content-center @endif">
                        ...
                        @else
                            <a href="{{ url('/panel/financial/account') }}" class="font-16 font-weight-bold text-dark-blue">Isi saldo akun</a>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="col-12 col-lg-3 mt-35">
                <a href="@if($authUser->isUser()) {{ url('/panel/webinars/purchases') }} @else {{ url('/panel/meetings/requests') }} @endif" class="dashboard-stats rounded-sm panel-shadow p-10 p-md-20 d-flex align-items-center">
                    <div class="stat-icon requests">
                        <img src="{{ asset('') }}assets/default/img/icons/request.svg" alt="">
                    </div>
                    <div class="d-flex flex-column ml-15">
                        ...
                        <span class="font-16 text-gray font-weight-500">{{ $authUser->isUser() ? 'Pelatihan diikuti' : 'Rapat tertunda' }}</span>
                    </div>
                </a>

                <a href="@if($authUser->isUser()) {{ url('/panel/meetings/reservation') }} @else {{ url('/panel/financial/sales') }} @endif" class="dashboard-stats rounded-sm panel-shadow p-10 p-md-20 d-flex align-items-center mt-15 mt-md-30">
                    <div class="stat-icon monthly-sales">
                        <img src="@if($authUser->isUser()) {{ asset('') }}assets/default/img/icons/meeting.svg @else {{ asset('') }}assets/default/img/icons/monay.svg @endif" alt="">
                    </div>
                    <div class="d-flex flex-column ml-15">
                        ...
                        <span class="font-16 text-gray font-weight-500">{{ $authUser->isUser() ? 'Rapat' : 'Penjualan bulanan' }}</span>
                    </div>
                </a>
            </div>

            <div class="col-12 col-lg-3 mt-35">
                <a href="{{ url('/panel/support') }}" class="dashboard-stats rounded-sm panel-shadow p-10 p-md-20 d-flex align-items-center">
                    <div class="stat-icon support-messages">
                        <img src="{{ asset('') }}assets/default/img/icons/support.svg" alt="">
                    </div>
                    <div class="d-flex flex-column ml-15">
                ...
                <a href="@if($authUser->isUser()) {{ url('/panel/webinars/my-comments') }} @else {{ url('/panel/webinars/comments') }} @endif" class="dashboard-stats rounded-sm panel-shadow p-10 p-md-20 d-flex align-items-center mt-15 mt-md-30">
                    <div class="stat-icon comments">
                        <img src="{{ asset('') }}assets/default/img/icons/comment.svg" alt="">
                    </div>
                    <div class="d-flex flex-column ml-15">
            ...
            <div class="col-12 col-lg-3 mt-35">
                <div class="bg-white account-balance rounded-sm panel-shadow py-15 py-md-15 px-10 px-md-20">
                    ...
                    <div class="mt-10 pt-10 border-top border-gray300 d-flex align-items-center justify-content-between">
                        ...
                        <span class="font-16 font-weight-bold text-secondary">{{ (!empty($nextBadge) and !empty($nextBadge['badge'])) ? $nextBadge['badge']->title : 'Tidak terdifinisikan' }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row">
                            ...
                            <div class="d-flex align-items-center justify-content-between">
                                <div>
                                    ...
                                    <div class="font-12 text-gray mt-5">
                                        <span class="mr-5">Dibuat oleh {{ $getUnreadNoticeboard->sender }}</span>
                                        |
                                ...
                                <div>
                                    <button type="button" data-id="{{ $getUnreadNoticeboard->id }}" class="js-noticeboard-info btn btn-sm btn-border-white">Selengkapnya</button>
            ...
            <div class="col-12 col-lg-6 mt-35">
                <div class="bg-white monthly-sales-card rounded-sm panel-shadow py-10 py-md-20 px-15 px-md-30">
                    <div class="d-flex align-items-center justify-content-between">
                        <h3 class="font-16 text-dark-blue font-weight-bold">{{ ($authUser->isUser()) ? 'Statistik belajar' : 'Penjualan bulanan' }}</h3>
...
@push('scripts_bottom')
    <script src="{{ asset('') }}assets/default/vendors/apexcharts/apexcharts.min.js"></script>
    <script src="{{ asset('') }}assets/default/vendors/chartjs/chart.min.js"></script>

    <script>
        var offlineSuccess = '{{ ('Status Anda berhasil diubah!') }}';
        var $chartDataMonths = @json($monthlyChart['months']);
        var $chartData = @json($monthlyChart['data']);
    </script>

    <script src="{{ asset('') }}assets/default/js/panel/dashboard.min.js"></script>
@endpush
