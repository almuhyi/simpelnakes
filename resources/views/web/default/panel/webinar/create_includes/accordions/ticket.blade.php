<li data-id="{{ !empty($ticket) ? $ticket->id :'' }}" class="accordion-row bg-white rounded-sm panel-shadow mt-20 py-15 py-lg-30 px-10 px-lg-20">
    <div class="d-flex align-items-center justify-content-between " role="tab" id="ticket_{{ !empty($ticket) ? $ticket->id :'record' }}">
        <div class="font-weight-bold text-dark-blue" href="#collapseTicket{{ !empty($ticket) ? $ticket->id :'record' }}" aria-controls="collapseTicket{{ !empty($ticket) ? $ticket->id :'record' }}" data-parent="#ticketsAccordion" role="button" data-toggle="collapse" aria-expanded="true">
            <span>{{ !empty($ticket) ? $ticket->title : 'Paket Harga Baru' }}</span>
        </div>
            ...
            @if(!empty($ticket))
                <div class="btn-group dropdown table-actions mr-15 font-weight-normal">
                    ...
                    <div class="dropdown-menu">
                        <a href="{{ url('') }}/panel/tickets/{{ $ticket->id }}/delete" class="delete-action btn btn-sm btn-transparent">Hapus</a>
                    </div>
                </div>
            @endif
    ...
    <div id="collapseTicket{{ !empty($ticket) ? $ticket->id :'record' }}" aria-labelledby="ticket_{{ !empty($ticket) ? $ticket->id :'record' }}" class=" collapse @if(empty($ticket)) show @endif" role="tabpanel">
        <div class="panel-collapse text-gray">
            <div class="js-content-form ticket-form" data-action="{{ url('') }}/panel/tickets/{{ !empty($ticket) ? $ticket->id . '/update' : 'store' }}">
                ...
                <div class="row">
                    <div class="col-12 col-lg-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="ajax[{{ !empty($ticket) ? $ticket->id : 'new' }}][title]" class="js-ajax-title form-control" value="{{ !empty($ticket) ? $ticket->title :'' }}" placeholder="Maksimal 64 karakter"/>
                        ...
                        <div class="form-group">
                            ...
                            @if($webinar->isWebinar() and empty($ticket) and !empty($webinar->capacity) and !empty($sumTicketsCapacities))
                                <span class="test-gray font-12 d-block">
                                    Tetap: <span class="js-ticket-remaining-capacity">{{ $webinar->capacity - $sumTicketsCapacities }}</span></span>
                            @endif

                            <input type="text" name="ajax[{{ !empty($ticket) ? $ticket->id : 'new' }}][capacity]" class="js-ajax-capacity form-control mt-10" value="{{ !empty($ticket) ? $ticket->capacity :'' }}" placeholder="{{ $webinar->isWebinar() ? 'Biarkan kosong untuk menggunakan kapasitas kelas' : 'Biarkan kosong untuk tidak terbatas.' }}"/>
                            <div class="invalid-feedback"></div>
                        </div>
                    </div>
                </div>
