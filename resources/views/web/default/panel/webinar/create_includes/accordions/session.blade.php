...
<li data-id="{{ !empty($chapterItem) ? $chapterItem->id :'' }}" class="accordion-row bg-white rounded-sm border border-gray300 mt-20 py-15 py-lg-30 px-10 px-lg-20">
    <div class="d-flex align-items-center justify-content-between " role="tab" id="session_{{ !empty($session) ? $session->id :'record' }}">
        <div class="d-flex align-items-center" href="#collapseSession{{ !empty($session) ? $session->id :'record' }}" aria-controls="collapseSession{{ !empty($session) ? $session->id :'record' }}" data-parent="#chapterContentAccordion{{ !empty($chapter) ? $chapter->id :'' }}" role="button" data-toggle="collapse" aria-expanded="true">
            ...
            <div class="font-weight-bold text-dark-blue d-block">{{ !empty($session) ? $session->title : 'Tambah sesi live baru' }}</div>
        </div>
        ...
        <div class="d-flex align-items-center">
            ...
            @if(!empty($session))
                <a href="{{ url('') }}/panel/sessions/{{ $session->id }}/delete" class="delete-action btn btn-sm btn-transparent text-gray">
    ...
    <div id="collapseSession{{ !empty($session) ? $session->id :'record' }}" aria-labelledby="session_{{ !empty($session) ? $session->id :'record' }}" class=" collapse @if(empty($session)) show @endif" role="tabpanel">
        <div class="panel-collapse text-gray">
            <div class="js-content-form session-form" data-action="{{ url('') }}/panel/sessions/{{ !empty($session) ? $session->id . '/update' : 'store' }}">
                ...
                <div class="row">
                    <div class="col-12 col-lg-6">
                        ...
                        <div class="form-group">
                            ...
                            <div class="js-session-api">
                                <div class="custom-control custom-radio custom-control-inline">
                                    ...
                                    <label class="custom-control-label" for="localApi{{ !empty($session) ? $session->id : '' }}">Custom</label>
                                </div>
                                ...
                                <div class="custom-control custom-radio custom-control-inline">
                                    ...
                                    <label class="custom-control-label" for="bigBlueButton{{ !empty($session) ? $session->id : '' }}">BigBlueButton</label>
                                </div>
                                ...
                                <div class="custom-control custom-radio custom-control-inline">
                                    ...
                                    <label class="custom-control-label" for="zoomApi{{ !empty($session) ? $session->id : '' }}">Zoom</label>
                                </div>
                                ...
                                @if(getFeaturesSettings('agora_live_streaming') and (!empty($webinar->price) or getFeaturesSettings('agora_in_free_courses')))
                                    <div class="custom-control custom-radio custom-control-inline">
                                        ...
                                        <label class="custom-control-label" for="agoraApi{{ !empty($session) ? $session->id : '' }}">Kelas live dalam aplikasi</label>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="js-zoom-not-complete-alert mt-10 text-danger d-none">
                                ...
                                <a href="{{ url('/panel/setting/step/8') }}" class="text-primary" target="_blank">Pergi ke pengaturan</a>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][title]" class="js-ajax-title form-control" value="{{ !empty($session) ? $session->title : '' }}" placeholder="Maksimal 255 karakter"/>
                            <div class="invalid-feedback"></div>
                        </div>
                        ...
                        <div class="form-group mt-20">
                            <div class="d-flex align-items-center justify-content-between">
                                <label class="cursor-pointer input-label" for="sessionStatusSwitch{{ !empty($session) ? $session->id : '_record' }}">Aktif</label>
                        ...
                        <div class="js-agora-chat-and-rec  {{ (empty($session) or $session->session_api !== 'agora') ? 'd-none' : '' }}">
                            @if(getFeaturesSettings('agora_chat'))
                                <div class="form-group mt-20">
                                    <div class="d-flex align-items-center justify-content-between">
                                        <label class="cursor-pointer input-label" for="sessionAgoraChatSwitch{{ !empty($session) ? $session->id : '_record' }}">Obrolan</label>
                                        <div class="custom-control custom-switch">
                        ...
                        @if(getFeaturesSettings('sequence_content_status'))
                            <div class="form-group mt-20">
                                <div class="d-flex align-items-center justify-content-between">
                                    <label class="cursor-pointer input-label" for="SequenceContentSwitch{{ !empty($session) ? $session->id : '_record' }}">Lanjutan</label>
                                    <div class="custom-control custom-switch">
                            ...
                            <div class="js-sequence-content-inputs pl-5 {{ (!empty($session) and ($session->check_previous_parts or !empty($session->access_after_day))) ? '' : 'd-none' }}">
                                <div class="form-group">
                                    <div class="d-flex align-items-center justify-content-between">
                                        <label class="cursor-pointer input-label" for="checkPreviousPartsSwitch{{ !empty($session) ? $session->id : '_record' }}">Paksa peserta untuk melewati bagian sebelumnya</label>
                                        <div class="custom-control custom-switch">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][access_after_day]" value="{{ (!empty($session)) ? $session->access_after_day : '' }}" class="js-ajax-access_after_day form-control" placeholder="misalnya '10' untuk mengizinkan peserta mengakses bagian ini setelah 10 hari."/>
                                    <div class="invalid-feedback"></div>
                                </div>
                            </div>
                        @endif
                ...
                <div class="mt-30 d-flex align-items-center">
                    ...
                    @if(!empty($session))
                        @if(!$session->isFinished())
                            <a href="{{ url($session->getJoinLink(true)) }}" target="_blank" class="ml-10 btn btn-sm btn-secondary">Bergabung</a>
                        @else
