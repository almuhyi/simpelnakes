<li data-id="{{ !empty($chapterItem) ? $chapterItem->id :'' }}" class="accordion-row bg-white rounded-sm border border-gray300 mt-20 py-15 py-lg-30 px-10 px-lg-20">
    <div class="d-flex align-items-center justify-content-between " role="tab" id="file_{{ !empty($assignment) ? $assignment->id :'record' }}">
        <div class="d-flex align-items-center" href="#collapseFile{{ !empty($assignment) ? $assignment->id :'record' }}" aria-controls="collapseFile{{ !empty($assignment) ? $assignment->id :'record' }}" data-parent="#chapterContentAccordion{{ !empty($chapter) ? $chapter->id :'' }}" role="button" data-toggle="collapse" aria-expanded="true">
            ...
            <div class="font-weight-bold text-dark-blue d-block">{{ !empty($assignment) ? $assignment->title . ($assignment->accessibility == 'free' ? " (". 'Gratis' .")" : '') : 'Tugas baru' }}</div>
        </div>
        ...
        <div class="d-flex align-items-center">
            ...
            @if(!empty($assignment))
                <a href="{{ url('') }}/panel/assignments/{{ $assignment->id }}/delete" class="delete-action btn btn-sm btn-transparent text-gray">
    ...
    <div id="collapseFile{{ !empty($assignment) ? $assignment->id :'record' }}" aria-labelledby="file_{{ !empty($assignment) ? $assignment->id :'record' }}" class=" collapse @if(empty($assignment)) show @endif" role="tabpanel">
        <div class="panel-collapse text-gray">
            <div class="js-content-form assignment-form" data-action="{{ url('') }}/panel/assignments/{{ !empty($assignment) ? $assignment->id . '/update' : 'store' }}">
                ...
                <div class="row">
                    <div class="col-12 col-lg-12">
                        ...
                        <div class="form-group">
                            <label class="input-label">Tenggat waktu (hari)</label>
                        ...
                        <div class="form-group">
                            <label class="input-label">Percobaan</label>
                        ...
                        <div class="js-assignment-attachments-items form-group mt-15">
                            ...
                            <div class="assignment-attachments-main-row js-ajax-attachments position-relative">
                                <div class="mt-10 p-10 border rounded">
                                    <div class="mb-10">
                                        <label class="input-label">Judul</label>
                                        <input type="text" name="ajax[{{ !empty($assignment) ? $assignment->id : 'new' }}][attachments][assignmentTemp][title]" class="form-control" placeholder="Maksimal 255 karakter"/>
                                    </div>
                                    ...
                                    <div class="input-group product-images-input-group">
                                        ...
                                        <input type="text" name="ajax[{{ !empty($assignment) ? $assignment->id : 'new' }}][attachments][assignmentTemp][attach]" id="attachments_assignmentTemp" value="" class="form-control" placeholder="pilih Lampiran"/>
                                    </div>
                                </div>
                                ...
                                @foreach($assignment->attachments as $attachment)
                                    <div class="js-ajax-attachments position-relative">
                                        <div class="mt-10 p-10 border rounded">
                                            <div class="mb-10">
                                                <label class="input-label">Judul</label>
                                                <input type="text" name="ajax[{{ $assignment->id }}][attachments][{{ $attachment->id }}][title]" value="{{ $attachment->title }}" class="form-control" placeholder="Maksimal 255 karakter"/>
                                            </div>
                                            ...
                                            <div class="input-group product-images-input-group">
                                                ...
                                                <input type="text" name="ajax[{{ $assignment->id }}][attachments][{{ $attachment->id }}][attach]" id="attachments_{{ $attachment->id }}" value="{{ $attachment->attach }}" class="form-control" placeholder="pilih Lampiran"/>
                                            </div>
                                        </div>
                        ...
                        <div class="form-group mt-20">
                            <div class="d-flex align-items-center justify-content-between">
                                <label class="cursor-pointer input-label" for="assignmentStatusSwitch{{ !empty($assignment) ? $assignment->id : '_record' }}">Aktif</label>
                        ...
                        @if(getFeaturesSettings('sequence_content_status'))
                            <div class="form-group mt-20">
                                <div class="d-flex align-items-center justify-content-between">
                                    <label class="cursor-pointer input-label" for="SequenceContentSwitch{{ !empty($assignment) ? $assignment->id : '_record' }}">Lanjutan</label>
                                    <div class="custom-control custom-switch">
                            ...
                            <div class="js-sequence-content-inputs pl-5 {{ (!empty($assignment) and ($assignment->check_previous_parts or !empty($assignment->access_after_day))) ? '' : 'd-none' }}">
                                <div class="form-group">
                                    <div class="d-flex align-items-center justify-content-between">
                                        <label class="cursor-pointer input-label" for="checkPreviousPartsSwitch{{ !empty($assignment) ? $assignment->id : '_record' }}">Paksa peserta untuk melewati bagian sebelumnya</label>
                                        <div class="custom-control custom-switch">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" name="ajax[{{ !empty($assignment) ? $assignment->id : 'new' }}][access_after_day]" value="{{ (!empty($assignment)) ? $assignment->access_after_day : '' }}" class="js-ajax-access_after_day form-control" placeholder="misalnya '10' untuk mengizinkan peserta mengakses bagian ini setelah 10 hari."/>
                                    <div class="invalid-feedback"></div>
                                </div>
                            </div>
                        @endif
