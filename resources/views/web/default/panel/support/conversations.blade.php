...
@section('content')
    <section>
        <h2 class="section-title">
            Ringkasan dukungan</h2>

        <div class="activities-container mt-25 p-20 p-lg-35">
            <div class="row">
                <div class="col-4 d-flex align-items-center justify-content-center">
                    <div class="d-flex flex-column align-items-center text-center">
                        <img src="{{ asset('') }}assets/default/img/activity/41.svg" width="64" height="64" alt="">
                        <strong class="font-30 text-dark-blue font-weight-bold mt-5">{{ $openSupportsCount }}</strong>
                        <span class="font-16 text-gray font-weight-500">Percakapan dibuka</span>
                    </div>
                </div>

                <div class="col-4 d-flex align-items-center justify-content-center">
                    <div class="d-flex flex-column align-items-center text-center">
                        <img src="{{ asset('') }}assets/default/img/activity/40.svg" width="64" height="64" alt="">
                        <strong class="font-30 text-dark-blue font-weight-bold mt-5">{{ $closeSupportsCount }}</strong>
                        <span class="font-16 text-gray font-weight-500">Percakapan ditutup</span>
                    </div>
                </div>
                ...
                <div class="col-4 d-flex align-items-center justify-content-center">
                    <div class="d-flex flex-column align-items-center text-center">
                        ...
                        <span class="font-16 text-gray font-weight-500">Total percakapan</span>
                    </div>
                </div>
    ...
    <section class="mt-25">
        <h2 class="section-title">Filter pesan</h2>

        <div class="panel-section-card py-20 px-25 mt-20">
            <form action="{{url('/panel/support') }}" method="get">
                <div class="row">
                    ...
                    @if(!$authUser->isUser())
                        <div class="col-12 col-md-4 col-lg-2">
                            <div class="form-group">
                                <label class="input-label">Jenis user</label>
                                <select class="form-control" id="userRole" name="role">
                                    <option value="all">Semua role</option>
                                    <option value="student" @if(request()->get('role') == 'student') selected @endif >Peserta</option>
                                    <option value="teacher" @if(request()->get('role') == 'teacher') selected @endif >Instruktur</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div id="studentSelectInput" class="col-12 col-md-4 col-lg-2 @if(request()->get('role') != 'student' and (empty(request()->get('student'))  or request()->get('student') == 'all')) d-none @endif">
                            <div class="form-group">
                                ...
                                <select name="student" class="form-control select2" data-placeholder="Semua">
                    ...
                    <div id="teacherSelectInput" class="col-12 col-md-4 col-lg-2 @if(!$authUser->isUser() and request()->get('role') != 'teacher' and (empty(request()->get('teacher')) or request()->get('teacher') == 'all')) d-none @endif">
                        <div class="form-group">
                            ...
                            <select name="teacher" class="form-control select2" data-placeholder="Semua">
                                <option value="all">Semua</option>
                    ...
                    <div class="col-12 col-md-4 col-lg-2">
                        <div class="form-group">
                            ...
                            <select name="webinar" class="form-control select2" data-placeholder="Semua">
                                <option value="all">Semua</option>

                                @foreach($webinars as $webinar)
                                    <option value="{{ $webinar->id }}" @if(request()->get('webinar') == $webinar->id) selected @endif>{{ $webinar->title }} @if(in_array($webinar->id,$purchasedWebinarsIds))  @endif</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-4 col-lg-2">
                        <div class="form-group">
                            ...
                            <select class="form-control" id="status" name="status">
                                ...
                                <option value="replied" @if(request()->get('status') == 'replied') selected @endif >Dijawab</option>
                            </select>
                        </div>
                    </div>
    ...
    <section class="mt-40">
        <h2 class="section-title">Riwayat Pesan</h2>
                    ...
                    <div id="conversationsList" class="col-12 col-lg-6 conversations-list">
                        <div class="table-responsive">
                            <table class="table table-md">
                                <tr>
                                    <th class="text-left text-gray font-14 font-weight-500">Kontak</th>
                                    <th class="text-left text-gray font-14 font-weight-500">Judul</th>
                                    <th class="text-center text-gray font-14 font-weight-500">Status</th>
                                </tr>
                                <tbody>
                                ...
                                @foreach($supports as $support)
                                    <tr class="@if(!empty($selectSupport) and $selectSupport->id == $support->id) selected-row @endif">
                                        <td class="text-left">
                                            <a href="{{ url('') }}/panel/support/{{ $support->id }}/conversations" class="">
                                                <div class="user-inline-avatar d-flex align-items-center">
                                                    <div class="avatar bg-gray200">
                                                        <img src="{{ asset((!empty($support->webinar) and $support->webinar->teacher_id != $authUser->id) ? $support->webinar->teacher->getAvatar() : $support->user->getAvatar()) }}" class="img-cover" alt="">
                                                    </div>
                                                    <div class="ml-10">
                                                        ...
                                                        <span class="mt-1 font-12 text-gray d-block">
                                                            {{ (!empty($support->webinar) and $support->webinar->teacher_id != $authUser->id) ? 'Instruktur' : ( ($support->user->isUser()) ? 'Peserta' : 'Staf') }}
                                                        </span>
                                                    </div>
                                                </div>
                                            </a>
                                        ...
                                        <td class="text-center align-middle">
                                            @if($support->status == 'close')
                                                <span class="text-danger font-weight-500 font-14">Ditutup</span>
                                            @elseif($support->status == 'supporter_replied')
                                                <span class="text-primary font-weight-500 font-14">Dijawab</span>
                                            @else
                                                <span class="text-warning font-weight-500 font-14">Menunggu</span>
                                            @endif
                                        </td>
                        ...
                        <div class="col-12 col-lg-6 border-left border-gray300">
                            <div class="conversation-box p-15 d-flex align-items-center justify-content-between">
                                <div>
                                    ...
                                    <span class="font-12 mt-1 text-gray d-block">Dibuat: {{ dateTimeFormat($support->created_at,'j M Y | H:i') }}</span>

                                    @if(!empty($selectSupport->webinar))
                                        <span class="font-12 text-gray d-block mt-5">Pelatihan: {{ $selectSupport->webinar->title }}</span>
                                    @endif
                                </div>

                                @if($selectSupport->status != 'close')
                                    <a href="{{ url('') }}/panel/support/{{ $selectSupport->id }}/close" class="btn btn-primary btn-sm">Tutup percakapan</a>
                                @endif
                            </div>
                                    ...
                                    @foreach($selectSupport->conversations as $conversations)
                                        <div class="rounded-sm mt-15 panel-shadow border p-15">
                                            <div class="d-flex align-items-center justify-content-between pb-20 border-bottom border-gray300">
                                                <div class="user-inline-avatar d-flex align-items-center">
                                                    <div class="avatar bg-gray200">
                                                        <img src="{{ asset((!empty($conversations->supporter)) ? $conversations->supporter->getAvatar() : $conversations->sender->getAvatar()) }}" class="img-cover" alt="">
                                                    </div>
                                                    <div class="ml-10">
                                                        ...
                                                        <span class="mt-1 font-12 text-gray d-block">{{ (!empty($conversations->supporter)) ? 'Staf' : $conversations->sender->role_name }}</span>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="d-flex flex-column align-items-end">
                                                    ...
                                                    @if(!empty($conversations->attach))
                                                        <a href="{{ url($conversations->attach) }}" target="_blank" class="font-12 mt-10 text-danger"><i data-feather="paperclip" height="14"></i> Lampiran</a>
                                                    @endif
                                                </div>
                                            </div>
                            ...
                            <div class="conversation-box mt-30 py-10 px-15">
                                <h3 class="font-14 text-dark-blue font-weight-bold">Balas pesan</h3>
                                <form action="{{ url('') }}/panel/support/{{ $selectSupport->id }}/conversations" method="post" class="mt-5">
                                    {{ csrf_field() }}
                    ...
                    @else
                        <div class="col-12 col-lg-6 border-left border-gray300">
                            @include(getTemplate() . '.includes.no-result',[
                                ...
                                'hint' => nl2br(('Pilih percakapan dukungan untuk menampilkan riwayat pesan.')),
                            ])
                        </div>
                    @endif
                </div>
        ...
        @else
            ...
            @include(getTemplate() . '.includes.no-result',[
                'file_name' => 'support.png',
                'title' => ('Anda tidak memiliki pesan dukungan'),
                'hint' => nl2br(('Anda dapat menghubungi instruktur dan tim dukungan platform.')),
            ])
