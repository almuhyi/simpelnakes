        ...
        <div class="d-flex flex-column h-100">
            <div class="mb-auto navbar-notification-card" data-simplebar>
                ...
                @if(!empty($unReadNotifications) and count($unReadNotifications))
                    ...
                    @foreach($unReadNotifications as $unReadNotification)
                        <a href="{{ url('') }}/panel/notifications?notification={{ $unReadNotification->id }}">
            ...
            @if(!empty($unReadNotifications) and count($unReadNotifications))
                <div class="mt-10 navbar-notification-action">
                    <a href="{{ url('/panel/notifications') }}" class="btn btn-sm btn-danger btn-block">Semua pemberitahuan</a>
                </div>
            @endif
        </div>
    </div>
