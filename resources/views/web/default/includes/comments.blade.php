<div class="mt-35">
    <h2 class="section-title after-line">Komentar <span class="ml-5">({{ $comments->count() }})</span></h2>
    ...
    @if($comments)
        @foreach($comments as $comment)
            <div class="comments-card shadow-lg rounded-sm border px-20 py-15 mt-30" data-address="{{ url('')}}/comments/{{ $comment->id }}/reply" data-csrf="{{ csrf_token() }}" data-id="{{ $comment->id }}">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div class="btn-group dropdown table-actions">
                            ...
                            <div class="dropdown-menu">
                                ...
                                {{-- <button type="button" data-item-id="{{ $inputValue }}"  data-comment-id="{{ $comment->id }}" class="btn-transparent webinar-actions d-block mt-10 text-hover-primary report-comment">Laporkan</button> --}}
                    ...
                    @foreach($comment->replies as $reply)
                        <div class="rounded-sm border px-20 py-15 mt-30">
                            <div class="d-flex align-items-center justify-content-between">
                                <div class="user-inline-avatar d-flex align-items-center mt-10">
                                    <div class="avatar bg-gray200">
                                        <img src="{{ asset($reply->user->getAvatar()) }}" class="img-cover" alt="">
                                    </div>
                                    <div class="d-flex flex-column ml-5">
                                ...
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="btn-group dropdown table-actions">
                                        ...
                                        <div class="dropdown-menu">
                                            <button type="button" class="btn-transparent webinar-actions d-block text-hover-primary reply-comment">Balas</button>
                                            {{-- <button type="button" data-item-id="{{ $inputValue }}" data-comment-id="{{ $reply->id }}" action="{{ url('')}}/comments/{{ $reply->id }}/report" class="btn-transparent webinar-actions d-block mt-10 text-hover-primary report-comment">Laporkan</button> --}}

                                            @if(auth()->check() and auth()->user()->id == $reply->user_id)
                                                <a href="{{ url('') }}/comments/{{ $reply->id }}/delete" class="webinar-actions d-block mt-10 text-hover-primary">Hapus</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
