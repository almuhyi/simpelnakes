...
@if(!empty($advertisingModalSettings))
    <div class="d-none" id="advertisingModalSettings">
        ...
        <div class="d-flex align-items-center justify-content-center">
            <img src="{{ asset($advertisingModalSettings['image'] ?? '') }}" class="img-fluid rounded-lg" alt="{{ $advertisingModalSettings['title'] ?? 'ads' }}">
        </div>
        ...
        <div class="row align-items-center mt-20">
            @if(!empty($advertisingModalSettings['button1']) and !empty($advertisingModalSettings['button1']['link']) and !empty($advertisingModalSettings['button1']['title']))
                <div class="col-6">
                    <a href="{{ url($advertisingModalSettings['button1']['link']) }}" class="btn btn-primary btn-sm btn-block">{{ $advertisingModalSettings['button1']['title'] }}</a>
                </div>
            @endif
            ...
            @if(!empty($advertisingModalSettings['button2']) and !empty($advertisingModalSettings['button2']['link']) and !empty($advertisingModalSettings['button2']['title']))
                <div class="col-6">
                    <a href="{{ url($advertisingModalSettings['button2']['link']) }}" class="btn btn-outline-primary btn-sm btn-block">{{ $advertisingModalSettings['button2']['title'] }}</a>
                </div>
            @endif
        </div>
    </div>
