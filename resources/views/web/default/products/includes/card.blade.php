    ...
    <figure>
        <div class="image-box">
            <a href="{{ $product->getUrl() }}" class="image-box__a">
                ...
                @elseif($hasDiscount)
                <span class="badge badge-danger">Diskon {{ $hasDiscount->percent }}%</span>
        ...
        <figcaption class="product-card-body">
            ...
            @if(!empty($product->category))
                <span class="d-block font-14 mt-10">Kategori <a href="/products?category_id={{ $product->category->id }}" target="_blank" class="text-decoration-underline">{{ $product->category->title }}</a></span>
            @endif
            ...
            <div class="product-price-box mt-25">
                @if(!empty($isRewardProducts) and !empty($product->point))
                    <span class="text-warning real font-14">{{ $product->point }} Poin</span>
